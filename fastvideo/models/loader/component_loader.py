...
class TransformerLoader(ComponentLoader):
    ...
    def load(self, model_path: str, fastvideo_args: FastVideoArgs):
        ...
        logger.info(
            "Transformer offload strategy: %s (%.2fGB in bf16)",
            _offload_str,
            total_params * 2 / 1e9,
        )
        ...
        if fastvideo_args.inference_mode and fastvideo_args.dit_layerwise_offload:
            # Check recursively for nn.ModuleList (some models nest
            # transformer blocks inside a wrapper module).
            has_module_list = any(
                isinstance(m, nn.ModuleList) for m in model.modules()
            )
            if has_module_list:
